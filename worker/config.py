...
ATTACH_MIN_COS = 0.45
# Насколько уверенно кластер должен напоминать человека из базы, чтобы это мнение
# запрещало слить его с кластером ДРУГОГО человека. Подсказка нужна там, где
# голоса похожи: без неё Яна и «алена психотерапевт» слипались намертво.
GUIDE_MIN_COS = 0.62
GUIDE_MIN_GAP = 0.05
# ПРЯМОЕ НАЗНАЧЕНИЕ по базе голосов, до всякой кластеризации. Кластеризация не
# знает, кто есть кто, и склеивает похожие голоса намертво: на записи #49 Яна и
# «алена психотерапевт» оказывались одним кластером при любом пороге, хотя по
# отдельности их реплики расходятся уверенно — 17 минут против 13. Реплика,
# похожая на известного человека, должна идти сразу к нему.
# Пороги проверены на 539 репликах, размеченных человеком: покрытие 53.6% при
# точности 99.3% (мягче — 61.8% при 97.6%, строже — 43.8% при 100%).
DIRECT_MIN_COS = 0.55
DIRECT_MIN_GAP = 0.10
...
CONF_OK = 0.70                # порог для КЛАСТЕРА: он усреднён, требования выше
TOP2_MARGIN = 0.15            # и отрыв от второго кандидата у кластера тоже
# Для ОДИНОЧНОЙ реплики пороги ниже: её вектор шумный, и абсолютная близость почти
# никогда не дотягивает до кластерной. Решает отрыв от второго кандидата — реплика
# с 0.58 против 0.27 опознана уверенно, хотя прежнее правило звало её спорной.
# Подобрано на 523 репликах, размеченных человеком: прежнее «0.70 и 0.15» давало
# 27% уверенных при 100% точности, новое — 55% при 98.9%.
UTT_CONF_OK = 0.55
UTT_TOP2_MARGIN = 0.10
