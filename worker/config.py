...
# Пороги подобраны на реальной записи (tune_speechmap.py): при gap=8/pad=4 карта
# сохраняет 99% настоящей речи и отсекает 31% записи. Более агрессивные значения
# начинают терять речь — этого не допускаем: потерянное слово дороже сэкономленного цента.
NO_SPEECH_MAX = 0.6
